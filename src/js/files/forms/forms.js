...
export function formFieldsInit() {
  ...
  document.body.addEventListener("click", function (e) {
    ...
    if (!target.hasAttribute("class") || !className.includes("select")) return;
    console.log(target);
    formValidate.validateSelect(
      target.closest(".select").querySelector("select")
    );
  });
}
// Валидация форм
export let formValidate = {
    ...
    if (formRequiredItems.length) {
      formRequiredItems.forEach((formRequiredItem) => {
        if (formRequiredItem.tagName === "SELECT") {
          this.validateSelect(formRequiredItem);
        } else if (
  ...
  validateInput(formRequiredItem) {
    ...
    if (formRequiredItem.dataset.required === "email") {
      ...
      if (this.emailTest(formRequiredItem)) {
        ...
        this.removeField(formRequiredItem);
        error++;
      } else {
        this.removeError(formRequiredItem);
        this.addField(formRequiredItem);
      }
    } else if (
      ...
      if (formRequiredItem.value.length === 10) {
        this.removeError(formRequiredItem);
        this.addField(formRequiredItem);
      } else {
        this.addError(formRequiredItem);
    ...
    } else if (formRequiredItem.value.length > 0) {
      this.removeError(formRequiredItem);
      this.addField(formRequiredItem);
    } else {
      this.addError(formRequiredItem);
  ...
  validateSelect(selectReqItem) {
    setTimeout(() => {
      if (selectReqItem.value == selectReqItem.dataset.unvalid) {
        this.addError(selectReqItem);
      } else {
        this.removeError(selectReqItem);
      }
    }, 100);
  },
  ...
  addField(formRequiredItem) {
    formRequiredItem.classList.add("_form-field");
    formRequiredItem.parentElement.classList.add("_form-field");
    let inputError =
      formRequiredItem.parentElement.querySelector(".form__field");
    if (inputError) formRequiredItem.parentElement.removeChild(inputError);
  },
  removeField(formRequiredItem) {
    formRequiredItem.classList.remove("_form-field");
    formRequiredItem.parentElement.classList.remove("_form-field");
  },
...
const jsModalValues = document.querySelector(".js-btn-form");

if (jsModalValues) {
  const btn = jsModalValues.querySelector(".js-btn-val");
  const inputsOr = jsModalValues.querySelectorAll("input");
  const childs = document.querySelector(".js-form-child");

  const inputsPop = childs.querySelectorAll("input");

  btn.addEventListener("click", (e) => {
    inputsOr.forEach((element, index) => {
      if (inputsPop[index]) {
        inputsPop[index].value = element.value;
      }
    });
  });
}
